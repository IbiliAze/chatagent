...
class TestEviction:
  ...
  async def test_never_exceeds_max_entries(self, cache: Cache) -> None:
    for i in range(10):
      await cache.set(f'query {i}', self.thread_id, f'response {i}')

    assert len(cache) == 3

  async def test_evicts_least_recently_used(self, cache: Cache) -> None:
...
class TestStats:
  ...
  async def test_hits_are_counted_per_entry(self, cache: Cache) -> None:
    await cache.set('query', self.thread_id, 'response')
    await cache.get('query', self.thread_id)
    final = await cache.get('query', self.thread_id, return_full=True)
